create procedure dbo.ExtractCovidIncidentLab
  @isRestart tinyint = 0
as
begin
  ...
  set nocount on;
  declare 
    @name sysname = 'i.covid.incidentlab',
  ...
  begin try
   ...
   insert internals.covidincidentlab
    (
    ...
    where 
      x.rn = 1 and
      x.Id in ( select DVPR_RowID from [$(PRD_APHIM_UODS)].dbo.DV_PHPersonalRecord with (nolock) where DVPR_DiseaseCode_ID = 543030  )
    option
  ...
  begin
    waitfor delay '00:01';
    execute dbo.ExtractCovidIncidentLab @isRestart = 1;
  end
