create procedure dbo.ExtractCovidOutbreakProcessStatusHistory 
  @isRestart tinyint = 0
as
begin
  ...
  set nocount on;
  declare 
    @name sysname = 'COVID_OUTBREAK_PROCESS_STATUS_HISTORY',
  ...
  begin try
    truncate table dbo.COVID_OUTBREAK_PROCESS_STATUS_HISTORY;
    insert dbo.COVID_OUTBREAK_PROCESS_STATUS_HISTORY 
    (
    ...
    select 
      OUTB_RowID, AUD_ID, AUD_OldValue, AUD_NewValue, AUD_ActionDate, AUD_Username
    from 
       dbo.COVID_OUTBREAK_PROCESS_STATUS_HISTORY_VIEW
    --order by
    --  OUTB_RowID, AUD_ID
    option
  ...
  begin
    waitfor delay '00:01';
    execute dbo.ExtractCovidOutbreakProcessStatusHistory @isRestart = 1;
  end
