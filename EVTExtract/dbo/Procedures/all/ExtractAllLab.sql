...
begin
  ...
  begin try
    ...
    select
      ...
      LABNAME, CLIA, LABADDR1, LABADDR2, LABADDR3, HL7TimestampOfMessage, TimestampMessageReceived,
      try_convert(int,case when l.IMPORTSTATUS like ('%Case%') then substring(l.IMPORTSTATUS,len('Attached to Case ID - ')+1,100) else l.INCIDENTID end ) as CaseIncidentId
    from
