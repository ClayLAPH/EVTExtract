create view dbo.uvw_NCOVPUICDPHUse as
select  
  NCOVPUICDPHUseID,
  NCOVPUICDPHUseRepLabConfDt = try_convert(datetime,NCOVPUICDPHUseRepLabConfDt ),
  NCOVPUICDPHUseRepPUIDt = try_convert(datetime,NCOVPUICDPHUseRepPUIDt),
  DIID = RECORD_ID,
...
( select  
  ...
  where   
    UDF.SECTION_DEF_DR = 'NCOVPUICDPHUse' and FORM_DEF_DR = 'NCOVPUI'
) PivotData
pivot 
( max(FIELD_VALUE)
  for FIELD_DEF_DR in 
  ( NCOVPUICDPHUseID,
    NCOVPUICDPHUseRepLabConfDt,
    NCOVPUICDPHUseRepPUIDt
  )
) PivotTable
