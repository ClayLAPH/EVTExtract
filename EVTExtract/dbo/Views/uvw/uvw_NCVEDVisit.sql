CREATE VIEW dbo.[uvw_NCVEDVisit]
AS
SELECT          
         (
            CASE 
                WHEN [NCVEDvisit]='No' THEN 'N' 
                WHEN [NCVEDvisit]='Yes' THEN 'Y' 
                WHEN [NCVEDvisit]='Unknown' THEN 'U' 
                ELSE NULL 
            END
        ) AS 'NCVEDvisit'
        , [RECORD_ID] as 'DIID'
...
FROM    (
            ...
            WHERE   UDF.[SECTION_DEF_DR] = 'NCVEDVisit'  AND [FORM_DEF_DR] = 'NCOVPUI'
        ) AS PivotData
PIVOT (
    MAX([FIELD_VALUE])
    FOR [FIELD_DEF_DR] IN (
