CREATE VIEW dbo.[uvw_NCVDIGenEpi]
AS
SELECT  
          try_CAST([NCVDIGenSpecCollDate] AS DATETIME) AS [NCVDIGenSpecCollDate]
		, [NCVDIGenBank]
		, [NCVDIGenTestType]
		, [NCVDIGISAIDID]
		, [NCVDISequenceID]
...
FROM    (
            ...
            WHERE   
              UDF.[SECTION_DEF_DR] = 'NCVDIGenEpi' AND [FORM_DEF_DR] = 'NCOVPUI'
        ) AS PivotData
PIVOT (
    MAX([FIELD_VALUE])
    FOR [FIELD_DEF_DR] IN (
        [NCVDIGenSpecCollDate]
		, [NCVDIGenBank]
		, [NCVDIGenTestType]
		, [NCVDIGISAIDID]
		, [NCVDISequenceID]
