create view dbo.uvw_NCOVPUIHomless as
select  
  substring(NCOVPUIHomlessCurrExp,1,1) NCOVPUIHomlessCurrExp,
  NCOVPUIHomlessLocation,
  NCOVPUIHomlessShltr,
  DIID = RECORD_ID,
...
( select
  ...
  where   
    UDF.SECTION_DEF_DR = 'NCOVPUIHomless'  and FORM_DEF_DR = 'NCOVPUI'
) PivotData
pivot 
( max(FIELD_VALUE)
  for FIELD_DEF_DR in 
  ( NCOVPUIHomlessCurrExp, 
    NCOVPUIHomlessLocation, 
    NCOVPUIHomlessShltr
  )
) PivotTable