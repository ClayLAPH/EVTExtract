CREATE VIEW dbo.[uvw_NCVPEH]
AS
SELECT  
        
        [NCVPEHOtherTypeStay]
        , try_CAST([NCVPEHStartDate] AS DATETIME) AS [NCVPEHStartDate]
        , [NCVPEHStartDateApprx2]
        , [NCOVPUIHomlessShltr]
        , [NCVPEHNotes]
        , [NCVPEHEndDateApprx]
        , [NCVPEHCity]
        , [NCVPEHZip]
        , [NCVPEHTypeStay]
        , try_CAST([NCVPEHEndDate] AS DATETIME) AS [NCVPEHEndDate]
        , [NCVPEHAddrss]
        , [NCVPEHLocName]
        , [RECORD_ID] as 'DIID'
...
FROM    (
            ...
            WHERE   
              UDF.[SECTION_DEF_DR] = 'NCVPEH' AND [FORM_DEF_DR] = 'NCOVPUI'
        ) AS PivotData
PIVOT (
    MAX([FIELD_VALUE])
    FOR [FIELD_DEF_DR] IN (
