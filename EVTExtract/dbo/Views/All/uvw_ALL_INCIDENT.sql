create view dbo.uvw_ALL_INCIDENT as
select
  ...
  case person.PER_ZIPPLUS4
    when '' then ' ' 
    else person.PER_ZIPPLUS4
  end                                       Zip_Plus_4, 
  ...
  incident.American_Indian_or_Alaska_Native__Specify, 
  incident.Race_Category__American_Indian_or_Alaska_Native, 
  incident.Asian__Specify, 
  incident.Race_Category__Asian, 
  incident.Black_or_African_American__Specify, 
  incident.Race_Category__Black_or_African_American, 
  incident.Native_Hawaiian_or_Other_Pacific_Islander__Specify, 
  incident.Race_Category__Native_Hawaiian_or_Other_Pacific_Islander, 
  incident.Other__Specify, 
  incident.Race_Category__Other, 
  incident.Unknown__Specify, 
  incident.Race_Category__Unknown, 
  incident.White__Specify, 
  incident.Race_Category__White,
                                            Outbreak_IDs =
  ( select 
      [$(PRD_APHIM_UODS)].dbo.STRCONCAT(o.OUTB_OUTBREAKID) 
    from 
      dbo.COVID_OUTBREAK_LINKED_INCIDENT l 
      inner join 
      dbo.COVID_OUTBREAK o on l.dvob_rowid = o.OUTB_RowID 
    where
      incident.PR_INCIDENTID = l.IncidentID )                                         

from    
  dbo.ALL_PERSON person with (nolock) 
  inner join 
  dbo.ALL_INCIDENT incident with (nolock)
  on 
