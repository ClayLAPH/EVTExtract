create view dbo.COVID_INCIDENT as
select  
  pr.PR_ROWID, 
  pr.PR_PHTYPE, 
  a.PR_LEGACY_ROWID, 
  ...
  per.White___Specify
from   
  internals.covidpersonalrecord pr with (nolock)
  inner join 
  internals.covidperson per with (nolock)
  on 
    pr.PR_PERSONDR = per.dvper_rowid
  inner join
  internals.covidacts a with (nolock)
  on
    a.PR_ROWID = pr.PR_RowID
  left outer join
  internals.covidincidentlab cl with (nolock)
  on
    a.PR_ROWID = cl.Id
