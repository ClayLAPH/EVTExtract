create view dbo.ALL_OUTBREAK_LINKED_CONTACT_VIEW as
...
from            
  ( select
      pr.DVPR_DiseaseCode_ID Disease,
    ...
    where
      ...
      ( select        
        ...
        WHERE        
          outb.DVOB_DiseaseCode_ID not in ( 543030, 544041, 509985 ) -- covid, sars2, unknown respiratory ;
          and
          pr.DVPR_TypeDR = 494681
      )
    union
    select 
      outb.DVOB_DiseaseCode_ID,
      outb.DVOB_RowID, 
    ...
    where
      ...
      ( select 
        ...
        from 
          [$(PRD_APHIM_UODS)].dbo.DV_Outbreak       outb with (nolock)
        where 
          DVOB_DiseaseCode_ID not in ( 543030, 544041, 509985 ) -- covid, sars2, unknown respiratory ;
      )
  ) u