...
CREATE FUNCTION [DBO].[MDF_UCS_FULLNAME_ACTOBSERVATION_BYPARENTACTID]
(@PARENT_ACTID INT, @VALUETYPE VARCHAR(100), @METACODE VARCHAR(50))
RETURNS VARCHAR(255)
AS
...
BEGIN                    
	RETURN
		(               
			...
			FROM A_ACT (NOLOCK) 
			INNER JOIN  A_OBSERVATION (NOLOCK) ON A_OBSERVATION.ID = A_ACT.ID 
