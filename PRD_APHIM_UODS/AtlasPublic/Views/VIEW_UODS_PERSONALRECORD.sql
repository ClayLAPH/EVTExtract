...
CREATE   VIEW [AtlasPublic].[VIEW_UODS_PERSONALRECORD]   
AS
    SELECT A.ID AS PR_ROWID,
    dbo.FN_GetUCSFullName(PR.DVPR_TypeDR) PR_PHTYPE,
    ...
    DI.DVPR_INCIDENTID AS PR_LINKEDINCIDENTID,
    ...
    [DBO].[MDF_UCS_FULLNAME_UCS_BYUCSID](PR.DVPR_FinalDisposition) AS PR_FINALDISPOSITION,
    PR.DVPR_FinalDisposition AS PR_FINALDISPOSITION_DR,
    ...
    -- -SKUMAR ISSUENBR:110616
    -- + DI Linked to CI
    LEFT JOIN A_ACTRELATIONSHIP LINKEDINCIDENT (NOLOCK) ON LINKEDINCIDENT.TARGET_ID = A.ID AND LINKEDINCIDENT.TYPECODE='DRIV' 
    AND LINKEDINCIDENT.ID IN
    (
    ...
    WHERE ACTREL_CI.METACODE='PR_IncidentDR' AND ACTREL_CI.TYPECODE='DRIV' AND ACTREL_CI.target_ID IN (LINKEDINCIDENT.TARGET_ID) 
    ORDER BY ACTREL_CI.typeCode_OrTx DESC, ACTREL_CI.SOURCE_ID DESC
    ) 
    LEFT JOIN [dbo].DV_PHPERSONALRECORD DI (NOLOCK) ON DI.DVPR_ROWID = LINKEDINCIDENT.SOURCE_ID
    -- - DI Linked to CI
        ---- +Suroy Issue#135926 10/01/2012
