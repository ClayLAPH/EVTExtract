...
CREATE VIEW [covid].[SARS2_OUTBREAK_LINKED_CONTACT]
AS
...
FROM            (SELECT        OUTB_RowID, CONT_RecordInstanceDR, CONT_LastName, CONT_FirstName
                          FROM            (SELECT DISTINCT 
                                                    ...
                                                    WHERE        (LPA_A1.source_ID IN
                                                                                  (SELECT        DVOB_RowID
                                                                                    ...
                                                                                    WHERE        (DVOB_DiseaseCode_ID IN (544041)))) AND (dbo.DV_PHPersonalRecord.DVPR_TypeDR IN (494681))) AS a
                          UNION
                          ...
                          WHERE        (DV_Outbreak_2.DVOB_RowID IN
                                                       (SELECT        DVOB_RowID
                                                         ...
                                                         WHERE        (DVOB_DiseaseCode_ID IN (544041))))) AS tmp
...
Begin DesignProperties = 
   ...
   End
End
', @level0type = N'SCHEMA', @level0name = N'covid', @level1type = N'VIEW', @level1name = N'SARS2_OUTBREAK_LINKED_CONTACT';


GO
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPaneCount', @value = 1, @level0type = N'SCHEMA', @level0name = N'covid', @level1type = N'VIEW', @level1name = N'SARS2_OUTBREAK_LINKED_CONTACT';
