create view [covid].[COVID_PROCESS_STATUS_HISTORY]
as
SELECT [LPA_A1].[recordid]   AS [PR_RowID], 
       [LPA_A2].[id]         AS [AUD_ID], 
...
WHERE  ( [LPA_A2].[columnname] = 'PR_ProcessStatusDR' 
         AND [LPA_A1].[recordid] IN (SELECT CAST([AtlasPublic].[view_uods_personalrecord].[pr_rowid] AS VARCHAR)
              FROM   [AtlasPublic].[view_uods_personalrecord] (nolock) 
              WHERE  ( 
                 [AtlasPublic].[view_uods_personalrecord].[pr_diseasecode_dr] = 
                 543030 )) )