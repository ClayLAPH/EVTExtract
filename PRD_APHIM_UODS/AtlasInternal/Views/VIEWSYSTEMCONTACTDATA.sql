...
CREATE   VIEW [AtlasInternal].[VIEWSYSTEMCONTACTDATA] 
AS
SELECT ACTCASE.ID AS DIID,  
       PERSON.ID AS CONTACTID,  
       TINS.extension AS INSTANCEID,  
       (CASE WHEN ACTCASE.metaCode='PR_ROWID' THEN 'DI' WHEN ACTCASE.metaCode='AI_ROWID' THEN 'AR' WHEN ACTCASE.metaCode='OB_RowID' THEN 'OB' END) AS RECORDTYPE,
       ...
       UCSSTATUS.FULLNAME AS RLENT_STATUSDR,  
        ...
        Part_CreatedPersonalRecord.[Act_ID]  [RLENT_PersonalRecordDR] ,
        RLENT_CreatedPersonalRecordInstanceID.[extension] [RLENT_PersonalRecordID],        
        RLENT_CreatedPersonalRecordType.[shortName]   [RLENT_PersonalRecordType],
        RLENT_CreatedCITurnedToDI.[source_ID]  [RLENT_ContactInvestigationLinkedIncidentDR] ,  
        RLENT_CreatedCITurnedToDIID.[extension]    [RLENT_ContactInvestigationLinkedIncidentID] 
    FROM   E_ENTITY PERSON  (NOLOCK)
       ...
       INNER JOIN A_Act ACTCASE (NOLOCK)  
            ON  ACTCASE.ID = IND.ACT_ID AND  ACTCASE.metaCode IN ('PR_ROWID','AI_ROWID','OB_RowID') AND ACTCASE.statusCode IN ('ACTIVE','COMPLETED')
        INNER JOIN T_InstanceIdentifier TINS (NOLOCK)              
            ON TINS.Act_ID=ACTCASE.ID AND (TINS.[root] LIKE '2.16.840.1.113883.3.33.4.2.2.11.1%' OR TINS.[root] LIKE '2.16.840.1.113883.3.33.4.2.2.11.8%' OR TINS.[root] LIKE '2.16.840.1.113883.3.33.4.2.2.11.7%')
       ...
       LEFT JOIN A_ACTRELATIONSHIP (NOLOCK) ON A_ACTRELATIONSHIP.SOURCE_ID= FOLDER.ID AND A_ACTRELATIONSHIP.TypeCode='PERT'
       ...
       LEFT JOIN T_ATTRIBUTE PRIODR (NOLOCK) ON MBR.ID=PRIODR.Role_ID AND PRIODR.NAME='RLENT_Priority' AND PRIODR.TYPE='CV'
       LEFT JOIN V_UNIFIEDCODESET UCSPRIORITY (NOLOCK) ON PRIODR.ValueCode_ID=UCSPRIORITY.ID  
            --NG 07/30/2010 ISSUE#83086      
       ...
       --NG 07/30/2010 ISSUE#83086     
        ...
        -- + Shreeram Sridharan: 215846:7/27/2017
        --Details of DI/CI created from this contact (PR_DR, PR_ID, PR_Type)
        LEFT JOIN [P_Participation] Part_CreatedPersonalRecord (nolock) ON  MBR.ID=Part_CreatedPersonalRecord.[Role_ID] AND (Part_CreatedPersonalRecord.[metaCode] = 'RLENT_PersonalRecordDR')  
        LEFT JOIN [A_Act] RLENT_CreatedPersonalRecordDR (nolock) ON  RLENT_CreatedPersonalRecordDR.[ID]=Part_CreatedPersonalRecord.[Act_ID] AND (RLENT_CreatedPersonalRecordDR.[statusCode] <> 'nullified')  
        LEFT JOIN [T_InstanceIdentifier] RLENT_CreatedPersonalRecordInstanceID (nolock) ON  RLENT_CreatedPersonalRecordDR.[ID]=RLENT_CreatedPersonalRecordInstanceID.[Act_ID] AND (RLENT_CreatedPersonalRecordInstanceID.[root] = '2.16.840.1.113883.3.33.4.2.2.11.1'+dbo.FN_GetSiteID()+'.1'  )   
        LEFT JOIN [A_PublicHealthCase] RLENT_CreatedPersonalRecordPHCase (nolock) ON  RLENT_CreatedPersonalRecordDR.[ID]=RLENT_CreatedPersonalRecordPHCase.[ID]  
        LEFT JOIN [V_UnifiedCodeSet] RLENT_CreatedPersonalRecordType (nolock) ON  RLENT_CreatedPersonalRecordType.[ID]=RLENT_CreatedPersonalRecordPHCase.[phRecordTypeCode_ID]  

        --Details of DI created from the CI by clicking 'Create Incident button ' this contact (PR_DR, PR_ID, PR_Type)
        LEFT JOIN [A_ActRelationship] RLENT_CreatedCITurnedToDI (nolock) ON  RLENT_CreatedPersonalRecordDR.[ID]=RLENT_CreatedCITurnedToDI.[target_ID] AND (RLENT_CreatedCITurnedToDI.[metaCode] = 'PR_IncidentDR' AND RLENT_CreatedCITurnedToDI.[typeCode_OrTx] = 'CREATE INCIDENT') 
        LEFT JOIN [A_Act] RLENT_CreatedCITurnedToDIDR (nolock) ON  RLENT_CreatedCITurnedToDIDR.[ID]=RLENT_CreatedCITurnedToDI.[source_ID] AND (RLENT_CreatedCITurnedToDIDR.[statusCode] <> 'nullified')  
        LEFT JOIN [T_InstanceIdentifier] RLENT_CreatedCITurnedToDIID (nolock) ON  RLENT_CreatedCITurnedToDIDR.[ID]=RLENT_CreatedCITurnedToDIID.[Act_ID] AND (RLENT_CreatedCITurnedToDIID.[root] = '2.16.840.1.113883.3.33.4.2.2.11.1'+dbo.FN_GetSiteID()+'.1'  ) 
        -- - Shreeram Sridharan: 215846:7/27/2017
